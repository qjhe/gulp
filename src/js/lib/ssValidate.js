...
mValidate.prototype={  //封装方法  
	...
	vSecName2:function(val,callback){//姓名验证 提示：姓名只能为汉字,数字、英文和分隔号“·”且不能以“·”开头或结尾,最大长度30
		var regExp1 = /^([A-Za-z\·]|[\u4E00-\u9FA5]|\d)+$/;//汉字或英文加‘.’
		var regExp2 = /^\·|\·$/;//不能以‘·’开头或结尾
		var len = (val.length>3)?true:false;
		var vm_ = regExp1.test(val)&&!regExp2.test(val)&&len;
		if(!vm_) callback&&callback();
		return vm_;
	},
	vCompnyName:function(val,callback){//公司名称验证 提示：只能为汉字,数字、英文,最大长度50
		var regExp1 = /^([A-Za-z]|[\u4E00-\u9FA5]|\d)+$/;//汉字或英文或数字
		var len = (val.length>5)&&(val.length<50)?true:false;
		var vm_ = regExp1.test(val)&&len;
		if(!vm_) callback&&callback();
		return vm_;
	},
	vTime:function(val,callback){
		var regExp1 = /^(\d{4})-(\d{2})-(\d{2})\s(\d{2})(:||：)(\d{2})$/;//YYYY-MM-DD-HH-MM格式
		var vm_ = regExp1.test(val)
		if(!vm_) callback&&callback();
		return vm_;
	},
	...
	validateLuhn:function(cardno){//验证银行卡号
		cardno = cardno.replace(/\s+/g,"")||0;
		if(!cardno) return false;

		var nonCheckCodeCardId = cardno.substring( 0, cardno.length - 1 );
		var bit = cardno.charAt( cardno.length - 1 );

		if( !/^(\d+)$/.test(nonCheckCodeCardId) ) return false;// 如果传的不是数字

		var chs = $.trim(nonCheckCodeCardId).split("");

		var luhmSum = 0;
		for ( var i = chs.length - 1, j = 0; i >= 0; i-- , j++ ) {
			var k = parseInt(chs[i]);
			if ( j % 2 == 0 ) {
				k *= 2;
		...
		var check = ( luhmSum % 10 == 0 ) ? '0' : ( 10 - luhmSum % 10 );
		return check == bit;
	}
