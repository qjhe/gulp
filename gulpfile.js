...
//生产环境执行seajs任务，开发环境执行script_copy任务   src + '/js/**/*'
//gulp.task('script_uglify', [yargs.r ? 'seajs' : 'script_copy'], function (cb) {
//        gulp.src([ dist + '/js/lib/**/*.js', dist + '/js/app/**/*.js' ])
//            .pipe(uglify({
//                mangle: {
...
//gulp.task('script_uglify', [yargs.r ? 'seajs' : 'script_copy'], function (cb) {
//    if (yargs.r) {
//        return gulp.src([ dist + '/js/lib/**/*.js', dist + '/js/app/**/*.js' ], {base: dist + '/js'})
//                    .pipe(uglify({
//                        mangle: {
//                            except: ['require', 'exports', 'module']//这几个变量不能压缩混淆，否则会引发seajs的一些意外问题
//                        }
...
gulp.task('default', [ 'clean', 'html','script_uglify','script_restore','script','build','watch','server']);
//gulp.task('keysearch', [ 'clean','html','seajs','script_uglify','script_restore','script','build','watch','server']);
